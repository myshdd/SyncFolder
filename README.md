Вот обновленный файл `README.md` с учетом последних изменений:

````markdown
# SyncFolder.ps1

Скрипт PowerShell для синхронизации нескольких пар папок с поддержкой:
...
- **Несколько пар папок**: Поддержка синхронизации нескольких пар папок через массив `FolderPairs`.
...
### Обязательный параметр `FolderPairs`

Начиная с версии 29.01.2026, скрипт использует только массив `FolderPairs` для задания пар папок. Старые форматы (`SourcePath`/`DestinationPath`, `SourcePath1`/`DestinationPath1` и т.д.) больше не поддерживаются.
...
```json
{
  ...
  "FolderPairs": [
    ...
    {
      "Source": "C:\\test_dir\\Project\\MyApp3",
      "Destination": "C:\\test_dir\\Backup\\MyApp3"
    }
  ],
  "SyncDirection": "Both",
...
- `FolderPairs` (`array[object]`, **обязательный**)
  Массив пар папок для синхронизации. Каждый объект содержит:
  - `Source` (`string`, **обязательный**) — путь к исходной папке;
  - `Destination` (`string`, **обязательный**) — путь к папке назначения.

- `SyncDirection` (`"LeftToRight" | "RightToLeft" | "Both"`, **обязательный**)
...
**Важно**: Параметры `-SourcePath` и `-DestinationPath` больше не поддерживаются. Все пары папок должны быть указаны в `settings.json` через массив `FolderPairs`.
...
- Источник: `Source` (из каждой пары в `FolderPairs`).
- Получатель: `Destination` (из каждой пары в `FolderPairs`).
- Действия:
...
- Источник: `Destination` (из каждой пары в `FolderPairs`).
- Получатель: `Source` (из каждой пары в `FolderPairs`).
...
Выполняется **два прохода** для каждой пары папок:
...
- Считается, что **левая папка (Source)** — «мастер».
- Логика:
...
- Считается, что **правая папка (Destination)** — «мастер».
...
### 3. Односторонняя синхронизация нескольких пар папок
...
```json
{
  "FolderPairs": [
    {
      "Source": "C:\\Data\\Project1",
      "Destination": "D:\\Backups\\Project1"
    },
    {
      "Source": "C:\\Data\\Project2",
      "Destination": "D:\\Backups\\Project2"
    }
  ],
...
- `D:\Backups\Project1` становится зеркалом `C:\Data\Project1`;
- `D:\Backups\Project2` становится зеркалом `C:\Data\Project2`.
...
### 5. Двусторонняя синхронизация с приоритетом Source для нескольких пар
...
```json
{
  "FolderPairs": [
    {
      "Source": "C:\\Main\\Data1",
      "Destination": "D:\\Mirror\\Data1"
    },
    {
      "Source": "C:\\Main\\Data2",
      "Destination": "D:\\Mirror\\Data2"
    }
  ],
  "SyncDirection": "Both",
...
Поведение для каждой пары:

- `Destination` **следует** за `Source`:
...
### 6. Использование через Планировщик заданий Windows

#### Вариант 1: Базовый запуск с `settings.json`

Если у вас есть настроенный `settings.json` в той же папке, что и скрипт, можно создать задачу в Планировщике с такими параметрами:

1. **Триггер** — по расписанию (ежедневно, при входе в систему и т.п.).
2. **Действие** — запуск программы:
   - **Программа/скрипт**: `powershell.exe`
   - **Аргументы**: `-ExecutionPolicy Bypass -File "D:\DEV\Powershell\SyncFolder\syncfolder.ps1"`

#### Вариант 2: Запуск с явным указанием конфигурационного файла

Если `settings.json` находится в другом месте:

**Аргументы**: `-ExecutionPolicy Bypass -File "D:\DEV\Powershell\SyncFolder\syncfolder.ps1" -SettingsPath "D:\Configs\my_sync_settings.json"`

#### Вариант 3: Запуск с консольным выводом (для отладки)

Для отладки задачи в Планировщике можно направить вывод в лог-файл:

**Аргументы**: `-ExecutionPolicy Bypass -File "D:\DEV\Powershell\SyncFolder\syncfolder.ps1" > "D:\Logs\sync_console.log" 2>&1`

#### Вариант 4: Запуск от имени другого пользователя

Если синхронизируемые папки требуют прав другого пользователя:

1. В Планировщике создайте задачу.
2. На вкладке **Общие**:
   - Установите «Выполнять для всех пользователей».
   - Отметьте «Выполнять с наивысшими правами» (если нужно).
   - Нажмите «Изменить пользователя или группу» и укажите учетную запись с необходимыми правами.
   - Установите «Запускать вне зависимости от регистрации пользователя».

#### Рекомендации для Планировщика

- **Рабочая папка**: Укажите папку со скриптом в качестве «Начать в» (Start in), чтобы корректно разрешались относительные пути в `settings.json`.
- **Триггеры**:
  - Для регулярной синхронизации используйте ежедневный/еженедельный триггер.
  - Для синхронизации при подключении внешнего диска — триггер «При событии» с соответствующим ID события Windows.
- **Условия**:
  - Для мобильных устройств (ноутбуки) отметьте «Запускать только при питании от электросети».
  - При синхронизации с сетевыми дисками отметьте «Запускать только при наличии сетевого подключения».
- **Параметры**:
  - «Не запускать несколько экземпляров задачи» — предотвратит конфликты при долгом выполнении.
  - «Останавливать задачу, выполнение которой длится более:» — установите разумный лимит времени (например, 2 часа).

#### Проверка выполнения задачи

1. Проверяйте лог-файлы, указанные в `LogPath` конфигурации.
2. В Планировщике просматривайте «Журнал всех выполненных задач» на вкладке «Журнал».
3. При проблемах с правами проверьте, что:
   - Учётная запись имеет доступ ко всем исходным и целевым папкам.
   - Учётная запись имеет права на запись в каталог логов (если используется `LogPath`).

#### Пример конфигурации для Планировщика

- **Имя задачи**: `Ежедневная синхронизация проектов`
- **Триггеры**: Ежедневно в 20:00
- **Действие**:
  - **Программа**: `powershell.exe`
  - **Аргументы**: `-ExecutionPolicy Bypass -File "D:\DEV\Powershell\SyncFolder\syncfolder.ps1"`
  - **Рабочая папка**: `D:\DEV\Powershell\SyncFolder`
- **Условия**:
  - Запускать только при питании от электросети
  - Останавливать задачу, выполнение которой длится более 3 часов
- **Параметры**: Не запускать несколько экземпляров задачи

### 7. Синхронизация только PDF, с исключением временных файлов и логированием

```json
{
  "FolderPairs": [
    {
      "Source": "C:\\Docs\\Current",
      "Destination": "D:\\Docs\\Archive"
    }
  ],
  "SyncDirection": "LeftToRight",
...
### 1. «Не указаны пары папок для синхронизации (в settings.json отсутствует параметр FolderPairs)»

**Причина:** В файле `settings.json` отсутствует или пуст массив `FolderPairs`.

**Решение:**

- Добавить массив `FolderPairs` в `settings.json` с хотя бы одной парой папок.
- Убедиться, что каждая пара содержит оба поля: `Source` и `Destination`.
